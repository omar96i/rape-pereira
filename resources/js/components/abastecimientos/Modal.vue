                    ...
                    <div class="card-body row">
                        ...
                        <div class="col-12 col-sm-6">
                            <div class="mb-3">
                                ...
                                <select class="form-select" v-model="aux.categoria" @change="getAlimentos()" :disabled="tipo == 'edit'">
...
export default{
    ...
    methods:{
        setData(tipo, alimento_id, departamento_id){
            if(tipo == 'edit'){
                this.loading_data = true
                this.getData(alimento_id, departamento_id)
                this.tipo = tipo
            }
            if(tipo == 'insert'){
                this.loading_data = true
        ...
        getData(alimento_id, departamento_id){
            axios.get(`/abastecimiento/get/data/${departamento_id}/${alimento_id}`).then(res=>{
                this.data = {
        ...
        getAlimentos(){
            axios.get(`/abastecimiento/get/alimentos/${this.aux.categoria}`).then(res=>{
                this.alimentos = res.data.alimentos
                this.loading_data = false
            })
        },
        action(){
            this.loading = true
            axios.post((this.tipo == 'insert') ? `/abastecimiento/store/${this.data.departamento_id}/${this.data.alimento_id}` : `/abastecimiento/update/${this.data.departamento_id}/${this.data.alimento_id}`, this.data).then(res=>{
                this.loading = false
                if(res.data.status){
